...
test.describe('FakeAgent deterministic flow', () => {
  ...
  test('full session: diagnostic → targeted → mastery → complete', async ({ request }) => {
    ...
    const r2 = await request.post(`/sessions/${session_id}/answer`, { data: { value: wrong } });
    expect(r2.status()).toBe(200);
    const s2 = await r2.json();
    expect(s2.phase).toBe('targeted');

    // 3. Correct targeted answer → mastery
    const { a: a2, b: b2 } = s2.question;
    const r3 = await request.post(`/sessions/${session_id}/answer`, { data: { value: a2 + b2 } });
    expect(r3.status()).toBe(200);
    const s3 = await r3.json();
    expect(s3.phase).toBe('mastery');

    // 4. Correct mastery answer → complete
    const { a: a3, b: b3 } = s3.question;
    const r4 = await request.post(`/sessions/${session_id}/answer`, { data: { value: a3 + b3 } });
  ...
  test('direct mastery path: diagnostic correct → mastery → complete', async ({ request }) => {
    ...
    const r2 = await request.post(`/sessions/${session_id}/answer`, { data: { value: correct } });
    const s2 = await r2.json();
    expect(s2.phase).toBe('mastery');

    const r3 = await request.post(`/sessions/${session_id}/answer`, { data: { value: s2.question.a + s2.question.b } });
    const s3 = await r3.json();
